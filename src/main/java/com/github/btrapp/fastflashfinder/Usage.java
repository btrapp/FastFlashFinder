...
public class Usage {
	public static void main(String[] args) throws FastFlashException {
		 ...
		 * die=null]
		 */
		ZeroZeroFlashInst zeroZeroFlash = new ZeroZeroFlashInst(-1, -2, 20, 20);
		XRangeDieFinder df = new XRangeDieFinder(zeroZeroFlash, dies);
		df.setDieEdgeLogic(DieEdgeMatchLogic.RIGHT_SIDE);

		// Look up some dies with wafer level coordinates:
		System.out.println(df.findFlashAndDie(0, 0));
		System.out.println(df.findFlashAndDie(19, 71));
